...
class Sliderimages extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('slider_image, slider_imagedesc, slider_imagelink, slider_id', 'required'),
			array('slider_id, order_id', 'numerical', 'integerOnly'=>true),
			array('slider_image', 'length', 'max'=>250),
			array('slider_imagedesc', 'length', 'max'=>500),
			array('slider_imagelink', 'length', 'max'=>500),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('image_id, slider_image, slider_imagedesc, slider_imagelink, slider_id, order_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('slider_imagelink',$this->slider_imagelink,true);
