...
class MailtemplateController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				// 'actions'=>array('delete','index','view','create','update','sendmail','suspend','activate','del','business'),
				'users'=>array('jag','artifex','suniel','shameem','riadh.h','spangel'),
			),
			array('deny',  // deny all users
				'users'=>array('*'),
			),
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['MailTemplate']))
		{
			...
			$model->template_create=date('Y-m-d H:i:s');
			if($model->save())
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['MailTemplate']))
		{
			$model->attributes=$_POST['MailTemplate'];
			$model->save();
            //$this->redirect(Yii::app()->createUrl('mailtemplate'));
	 ...
	 * @param MailTemplate $model the model to be validated
	 */
